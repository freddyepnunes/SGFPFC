...
const Login = () => { 
  ...
  const handleSubmit = async (e) => {
    ...
    if (!username || !password) {
      setErrorMessage("E-mail e senha são obrigatórios."); // Exibe a mensagem de erro caso o email ou a senha não estão devidamente preenchidos
      return;
    }

    try {
      // Envia os dados para o servidor
      const response = await fetch("http://localhost:5000/api/login", { //Envia uma requisição para o servidor para fazer a validação do usuário que já está cadastrado no banco
      ...
      let data; // Declara a variável 'data' que armazenará a resposta processada do servidor
      try {
        // Tenta converter a resposta do servidor em formato JSON
        data = await response.json();
      } catch (jsonError) {
        // Caso ocorra um erro ao tentar converter a resposta para JSON, exibe uma mensagem de erro
